...
bool helper_for_starting_indices(const Config &cfg,
  ...
  for (int start_index : starting_indices) {
    ...
    if (cfg.maximum_num_of_finds != 0 &&
        find_count >= cfg.maximum_num_of_finds) {
      return true;
    }
  }
  return false;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < flags.size(); i++) {
    ...
    if (flag.find('v') != std::string::npos) {
      std::cout << "\033[36mgrep++\033[0m version 1.7.4\n";
      return 0;
    }
    ...
    cfg.show_line_nums =
        flag.find('n') != std::string::npos || cfg.show_line_nums;
    cfg.case_insensitive =
        flag.find('i') != std::string::npos || cfg.case_insensitive;
    cfg.just_display_find_count =
        flag.find('C') != std::string::npos || cfg.just_display_find_count;
    cfg.display_find_count =
        flag.find('c') != std::string::npos || cfg.display_find_count;

    int temp = handle_capital_N_flag(i, flags);
    if (temp == -1) {
      std::cout << "\033[31mERROR:\033[0m The flag '-N' must take an integer "
                   "argument after\n"
  ...
  while (std::getline(file, line)) {
    ...
    bool stop_loop = helper_for_starting_indices(cfg, starting_indices, line,
                                                 line_num, find_count);
    if (stop_loop) {
      break;
    }
